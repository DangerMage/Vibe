...
class MessageFilter(commands.Cog):
    ...
    async def on_message(self, message):
        if message.guild is None or message.author == self.bot.user:
            return
        if message.channel.id == 720394587792867378 or message.author == self.bot.user and not await bot_global.is_bot_manager(message.author):
    ...
    async def filter_message(self, message):
        if message.author.id in bot_global.custom_config.get("filter", "bypass"):
            return
        if any(role.id in bot_global.custom_config.get("filter", "bypass") for role in message.author.roles):
            return
